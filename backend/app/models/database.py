...
class LessonPodcast(Base):
    """Stores generated podcast audio for each lesson × language combination."""
    __tablename__ = "lesson_podcasts"

    id = Column(Integer, primary_key=True, index=True)
    lesson_id = Column(Integer, ForeignKey("lessons.id"), nullable=False, index=True)
    language = Column(String(10), nullable=False, default="en")  # en, hi, ta, te, bn, mr, etc.
    # Podcast script generated by LLM (conversational / storytelling format)
    podcast_script = Column(Text, nullable=True)
    # Relative path to the generated audio file (e.g. "podcasts/lesson_1_en.mp3")
    ...
    lesson = relationship("Lesson", backref="podcasts")


class Goal(Base):
    """User financial goals for tracking savings, investments, etc."""
    __tablename__ = "goals"

    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), index=True, nullable=False)
    title = Column(String(255), nullable=False)
    category = Column(String(50), default="savings")  # savings, investment, emergency, education, other
